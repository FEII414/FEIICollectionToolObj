...
- (void)setupViews{
    ...
    UIColor *lineColor = CQColor(83, 83, 83);
    self.frame = CGRectMake(0, 0, 110, 50);
...
//    self.layer.borderColor = [lineColor CGColor];
    ...
    [self setupButton:_decreaseBtn normalImage:@"minusTea" HighlightImage:@"minusTea"];
    [self addSubview:_decreaseBtn];
    
    _increaseBtn = [[UIButton alloc] init];
    [self setupButton:_increaseBtn normalImage:@"addTea" HighlightImage:@"addTea"];
...
- (void)decrease{
    ...
    if (newNum > 0) {
        _textField.text = [NSString stringWithFormat:@"%i", newNum];
        self.cancleBack(_textField.text);
    } else {
        self.cancleBack(@"dimiss");
    }
}
